...
const VALID_STATES = AppConfig.states;
const UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

export function calculateProjectPercentage(project) {
...
export function isDelayed(item) {
...
export function isPhaseOutsideProjectRange(phase, project) {
...
export function isMilestoneOutsideProject(phase, project) {
...
export function isPhaseOnHoliday(phase) {
  if (!phase.dataInizio || !phase.dataFine) return false;
...
export function hasPercentage100NotCompleted(item) {
  const percentage = item.percentualeCompletamento;
  if (percentage !== 100) return false;
  return item.stato !== 'completato';
}

export function autoFixPercentage100(item) {
  if (!AppConfig.logic.enableAutoFixPercent100ToCompleted) return false;
...
export function getProjectAlerts(project) {
  const alerts = {
...
export function getProjectAlertSeverity(alerts) {
...
export function getPhaseColor(phaseName, allPhaseNames = []) {
  if (!phaseName) return '#64748b';

  const preset = AppConfig.phasePresets.find(p => p.nome === phaseName);
  if (preset) {
    return preset.colore || null;
  }

  const customPhases = allPhaseNames
    .filter(name => !AppConfig.phasePresets.some(p => p.nome === name))
    .sort();

  const index = customPhases.indexOf(phaseName);
  if (index >= 0) {
    const colorIndex = index % AppConfig.customPhaseColors.length;
    return AppConfig.customPhaseColors[colorIndex];
  }

  return '#64748b';
}

export function getAllPhaseNames(projects) {
  const names = new Set();
...
export function createNewPhase(name = '', options = {}) {
...
export function validateAndFixProjects(projects) {
  const errors = [];
...
export function getPhasesSummary(project) {
  if (!Array.isArray(project.fasi)) {
    return {
      total: 0,
      completed: 0,
