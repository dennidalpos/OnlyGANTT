...
export function ProjectForm({
  ...
  useEffect(() => {
    if (initialPhaseId && activeTab === 'phases') {
      const timer = setTimeout(() => {
        const el = document.getElementById(`phase-card-${initialPhaseId}`);
        if (el) {
      ...
      <div className="slide-over-drawer" onClick={(e) => e.stopPropagation()}>
        <div className="drawer-header">
          <h3 className="drawer-title">
            <span>✏</span> {project ? 'Modifica Progetto' : 'Nuovo Progetto'}
          </h3>
          <button type="button" className="drawer-close-btn" onClick={onCancel} aria-label="Chiudi">
            ✕
          </button>
        </div>
        ...
        <div className="drawer-body">
          ...
          <div className="tabs-header">
            ...
            >
              Fasi & Milestone ({formData.fasi.length})
            </button>
          </div>
          ...
          {activeTab === 'general' && (
            <div className="tab-content">
              ...
              <div className="form-row">
                <div className="form-group">
                  <label htmlFor={`${projectFormId}-colore`}>Colore Progetto</label>
                  <input
                    id={`${projectFormId}-colore`}
                    type="color"
                    value={formData.colore || '#3b82f6'}
                ...
                <div className="form-group">
                  <label htmlFor={`${projectFormId}-stato`}>Stato Progetto</label>
                  <select
          ...
          {activeTab === 'phases' && (
            <div className="tab-content">
              {!readOnly && (
                <div style={{ marginBottom: '1rem' }}>
                  <div style={{ fontSize: '0.85rem', color: '#cbd5e1', marginBottom: '0.5rem' }}>
                    Aggiungi Fase Preset:
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                    {AppConfig.phasePresets.map(preset => (
                      <button
                        key={preset.nome}
                        type="button"
              ...
              {formData.fasi.map((fase, index) => {
                ...
                return (
                  <React.Fragment key={fase.id || index}>
                    <div
                      id={fase.id ? `phase-card-${fase.id}` : undefined}
                      style={{
                        padding: '1rem',
                        backgroundColor: 'var(--bg-tertiary)',
                        borderRadius: 'var(--radius-md)',
                        marginBottom: '0.75rem',
                        border: fase.id && fase.id === initialPhaseId ? '2px solid var(--accent-primary)' : '1px solid var(--border-color)',
                        boxShadow: fase.id && fase.id === initialPhaseId ? '0 0 10px rgba(59, 130, 246, 0.3)' : 'none'
                      }}
                    >
        ...
        <div className="drawer-footer">
          <button type="button" onClick={() => handleSave(false)} className="btn-success" disabled={readOnly || isSaving}>
            {isSaving ? 'Salvataggio...' : 'Salva e chiudi'}
          </button>
          {project && !readOnly && (
            <button type="button" onClick={handleDelete} className="btn-danger" disabled={isSaving}>
              Elimina
            </button>
          )}
