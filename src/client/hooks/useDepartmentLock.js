...
export function useDepartmentLock(department, userName, enabled) {
  ...
  const startHeartbeat = useCallback(() => {
    ...
    const scheduleHeartbeat = async () => {
      const baseMs = AppConfig.lock.heartbeatMinutes * 60 * 1000;
      const jitterMs = AppConfig.lock.heartbeatJitterMs || 0;
  ...
  const acquireLock = useCallback((force = false) => {
    ...
    }, AppConfig.lock.acquireDebounceMs);
  ...
  useEffect(() => {
    ...
    const pollStatus = async () => {
      ...
      try {
        ...
        if (isActive) {
          statusPollTimeoutRef.current = setTimeout(pollStatus, AppConfig.lock.statusPollMs);
        }
      }
    };
...
export default useDepartmentLock;
