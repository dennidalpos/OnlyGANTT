...
Update `scripts/script.md` when public scripts are added, removed, or renamed.

---

## 9. Verification

When behavior changes, add or update tests using the existing framework under `tests/`.
...
- `npm run bootstrap`: Runs `npm ci` to install dependencies. (Note: if you modify `package.json`, run `npm install` first to sync `package-lock.json`).
- `npm run doctor`: Validates the local environment and prerequisites.
